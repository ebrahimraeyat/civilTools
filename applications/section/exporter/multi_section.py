...
class MultiSection(multi_section_base, multi_section_window):
    ...
    def add_width(self):
        ...
        if self.plate_lengths.findItems(str(item), Qt.MatchContains):
            return
        self.plate_lengths.addItem(str(self.width_box.value()))

    def add_thick(self):
        item = self.width_box.value()
        if self.plate_thicks.findItems(str(item), Qt.MatchContains):
            return
        self.plate_thicks.addItem(str(self.thick_box.value()))

    def add_dist(self):
        item = self.dist_box.value()
        if self.section_dist.findItems(str(item), Qt.MatchContains):
            return
